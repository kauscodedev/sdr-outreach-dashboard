...
>
> **V3 update (2026-07-14):** the board now lives on the **Intelligence** page (nav renamed; same
> `/attention` route) and each watch row carries a grounded **Account Brief** (`lib/agent/briefs.ts`
> → `sdr_agent_briefs`, rendered by `BriefBlock`) generated by a tool-using loop with pgvector
> semantic recall over the call/email content corpus. Rep names come from the DB roster via props
> (the `REPS` config import is gone), and every row links to the account History panel
> (`AccountTimeline`). See `docs/AI-CRM-BLUEPRINT.md` for the full V3 picture.
